...
export class RecordComponent {
  ...
  if (this.busquedaCodigo) {
    if (this.filtroCargo === 'DNI') {
      ...
      resultados = resultados.filter(u => 
        u["Nombre y Apellido"].toLowerCase().includes(this.busquedaCodigo.toLowerCase())
      );
    } else if (this.filtroCargo === 'Correo Electrónico') {
      resultados = resultados.filter(u => 
        u["Correo Electrónico"].toLowerCase().includes(this.busquedaCodigo.toLowerCase())
      );
    }
  }
  ...
  const confirmado = confirm(`¿Estás seguro de eliminar al usuario ${usuario['Nombre y Apellido']}?`);
  ...
  try {
    ...
    const { success, error } = await this.usersService.eliminarUsuarioCompleto(
      usuario.idUsuario,
      usuario.idEmpleado,
      usuario.idAuth
    );
    ...
    alert(`Error técnico al eliminar: ${error instanceof Error ? error.message : 'Error desconocido'}`);
  }
}
  ...
  await this.cargarUsuarios();
  this.mostrarMensajeExito(this.usuarioParaEditar ? 
    'Usuario actualizado exitosamente' : 'Usuario creado exitosamente');
  this.modalAbierto = false;
  this.usuarioParaEditar = null;
}
