...
export class DetailsComponent implements OnInit {
  ...
  async ngOnInit() {
    if (this.pago?.idPedido) {
      try {
        this.productos = await this.receiptsService.obtenerDetallePedido(this.pago.idPedido);
      } catch (error) {
  ...
  obtenerNombreCliente(dni: string) {
    ...
    this.apiPeruService.buscarPorDni(dni)
      .pipe(
        finalize(() => this.cargandoCliente = false)
      )
      .subscribe({
        next: (response) => {
